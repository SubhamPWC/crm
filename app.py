...
# Geocode one text -> (lat, lon)
def ors_geocode(q: str, api_key: str, country_bias: str|None=None):
    try:
...
with st.sidebar:
    ...
    st.caption('CSV should have at least a **Location** column.')
    st.divider()
...
# ---- Determine rows needing geocode ----
    ...
    if not ORS_API_KEY:
        ...
        if total == 0:
            ...
            for i,(idx,row) in enumerate(todo.iterrows(), start=1):
                # Build query using Location (+ Customer if present)
                parts = [str(row.get('Location',''))]
                if 'Customer' in base.columns:
                    parts.append(str(row.get('Customer','')))
                q = ' '.join([p for p in parts if p and p.lower() != 'nan'])
...
with k4:
    ...
    st.metric('Total Qty', int(qty_series.sum()) if not qty_series.empty else 0)
...
with colA:
    ...
    if 'Customer' in filtered and not filtered.empty:
        ...
        if not vc.empty:
            ...
            chart = (alt.Chart(vc.head(15)).mark_bar(color='#7aa2ff')
...
with colB:
    ...
    if 'Package' in filtered and not filtered.empty:
        ...
        if not vc.empty:
            ...
            chart = (alt.Chart(vc.head(15)).mark_bar(color='#9fe6a0')
                     .encode(x=alt.X('Count:Q', title='Count'),
...
if {LAT,LON}.issubset(filtered.columns) and filtered[LAT].notna().any() and filtered[LON].notna().any():
    ...
    def status_color(s):
        ...
        return [120,200,255]
    mdf['__color__'] = mdf['Current Status'].apply(status_color) if 'Current Status' in mdf.columns else [120,200,255]

    # Tooltip text - use single-line string with 
 to avoid syntax errors
    ...
    view = pdk.ViewState(latitude=float(pd.to_numeric(mdf[LAT]).mean()),
                         longitude=float(pd.to_numeric(mdf[LON]).mean()), zoom=4)
    st.pydeck_chart(pdk.Deck(layers=[layer], initial_view_state=view, map_style='dark', tooltip=tooltip))
else:
    st.warning('Map needs numeric lat/lon values. Use the geocode button above to populate them from Location.')
